...
export default function PrintWizard({ isWholesale = false }) {
  ...
  function getCalculatedPrice() {
    ...
    if (isWholesaleActive) {
      // B2B Wholesale preserves existing GST logic
      gst = subtotal * 0.18;
      grandTotal = Math.round(subtotal + gst + courierCost);
    } else {
      // Normal Print: GST is completely 0 (Final Amount = Subtotal + Courier Charges)
      gst = 0;
            ...
            <div>
              ...
              >
                ...
                <div style={{ fontSize: 13, color: 'var(--on-surface-variant)' }}>
                  {uploading ? 'Uploading to Winstar Cloud...' : file ? `${(file.size / (1024 * 1024)).toFixed(2)} MB • Click to replace` : 'Supported: PDF, DOCX, JPG, PNG, CorelDRAW (.CDR)'}
                </div>
